/**
 * Voice Controller
 * Handles business logic for Twilio voice interactions
 */
...
/**
 * Join a conference call (for mobile app)
 */
const joinConference = async (req, res) => {
  try {
    const { to, conferenceId = 'MyConference', from = process.env.TWILIO_PHONE_NUMBER } = req.body;
    ...
    // Make the call and add to conference
    const call = await client.calls.create({
      to,
      from,
      url: `${process.env.BASE_URL}/api/voice/conference?conferenceId=${conferenceId}`,
      method: 'POST',
    });
